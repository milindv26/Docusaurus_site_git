...
>$$
>F(\bold{x'}_i\beta)=\frac{e^{\bold{x'}_i\boldsymbol{\beta}}}{1+e^{\bold{x'}_i\boldsymbol{\beta}}}.
>$$
...
$$
f_i= [F(\bold{x'}_i\beta)]^y \cdot [1- F(\bold{x'}_i\beta)]^{1-y},
$$
where $y\in\{0,1\}$.

Log likelihood function, $\log f_i$
$$
\log f_i= y_i\log[F(\bold{x'}_i\beta)] + (1-y_i)\log[1- F(\bold{x'}_i\beta)],
$$
take summation both sides,
$$
\sum_{i=1}^n\log f_i= \sum_{i=1}^n \bigg[y_i\log[F(\bold{x'}_i\beta)] + (1-y_i)\log[1- F(\bold{x'}_i\beta)]\bigg].
$$
